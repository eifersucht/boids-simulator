...
export const BoidPositionFragmentShader = `
uniform float clock;
...
void main() {
    ...
    wcoordinate = mod( 
        ( wcoordinate + del_change * 2.0 +
        length(velocity.xz) * del_change * 3.0 +
        max(velocity.y, 0.0) * del_change * 6.0 ), 
        50.0 
    );

    gl_FragColor = vec4(position + velocity * del_change * 15.0, wcoordinate);
}
`;